...
#define baoliuziSum  8
char *baoliuzi[baoliuziSum]={ "if","else","for","while","do","int","read","write"};
//下面定义纯单分界符，如需要可添加
char danfenfu[20]="+-*(){};,:";
//下面定义双分界符的首字符
char shuangfenfu[10]="><=!";
...
int TESTscan()//词法分析函数
{
   ...
   //scanf("%s",Scanout);
   if ((fin=fopen("aaa.t","r"))==NULL) //判断输入文件名是否正确
   {
      printf("\n打开词法分析输入文件出错!\n");
      return(1);//输入文件出错返回错误代码1
   }
   if ((fout=fopen("bbb.t","w"))==NULL) //判断输出文件名是否正确
   {
      printf("\n创建词法分析输出文件出错!\n");
   ...
   while(ch!=EOF)
   {
      ...
      if (isalpha(ch))   //如果是字母，则进行标识符处理
      {
		 ...
		 //查保留字
		 n=0;
		 while ((n<baoliuziSum) && strcmp(token,baoliuzi[n])) n++;
		 if (n>=baoliuziSum)  //不是保留字，输出标识符
			fprintf(fout,"%s\t%s\n","ID",token);  //输出标识符符号
		else//是保留字，输出保留字
			fprintf(fout,"%s\t%s\n",token,token);  //输出保留字符号
	  } else if (isdigit(ch))//数字处理
      {
		 ...
		 token[j]='\0';  //整数组合结束		 
		 fprintf(fout,"%s\t%s\n","NUM",token);  //输出整数符号
      } else if (strchr(danfenfu,ch)>0)  //单分符处理
      { 
		 token[0]=ch; token[1]='\0';
		 ch=getc(fin);//读下一个符号以便识别下一个单词
		 fprintf(fout,"%s\t%s\n",token,token);  //输出单分界符符号
      }else if (strchr(shuangfenfu,ch)>0)  //双分界符处理
      {
		 token[0]=ch;
