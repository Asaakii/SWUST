...
for i in range(12):
    ...
    url = start_url + str(20 * i) + '&limit=20&sort=new_score&status=P&percent_type=h'  # 确定要爬取的入口链接，差评把最后m改l，好评改h
    headers = {
    ...
    for j in range(len(outtime)):
        try:
            ...
            with open('好评.xlsx', 'a+', encoding='utf-8-sig', newline='') as datacsv:
