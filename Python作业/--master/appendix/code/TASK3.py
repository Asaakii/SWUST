...
for i in range(12):
    ...
    url = start_url + str(20 * i) + '&limit=20&sort=new_score&status=P&percent_type=l'  # 确定要爬取的入口链接
    headers = {
    ...
    for j in range(len(outtime)):
        try:
            ...
            with open('差评.xlsx', 'a+', encoding='utf-8-sig', newline='') as datacsv:
